from plugins.drm_handler import process_drm, STOP_DOWNLOADS
import requests
import asyncio
import aiohttp
import json
...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8565564772:AAGGhUhbyx81OYlpI8ocnaY7g6CHVNNmUI4")
...
async def start(bot, message):
  ...
  keyboard = [
    ...
    [
      InlineKeyboardButton("🛡️ DRM Downloader", callback_data="drm")
    ]
  ]
...
@bot.on_callback_query(filters.regex("^drm$"))
async def drm_callback(bot, callback_query):
    user_id = callback_query.from_user.id
    await callback_query.answer()
    if user_id not in auth_users:
        await bot.send_message(callback_query.message.chat.id, "⛔️ Access Denied")
        return
    await process_drm(bot, callback_query.message, user_id)

@bot.on_message(filters.command("stop"))
async def stop_command(bot, message):
    user_id = message.from_user.id
    STOP_DOWNLOADS.add(user_id)
    await message.reply_text("⛔️ अगला वीडियो डाउनलोड नहीं किया जाएगा।")
